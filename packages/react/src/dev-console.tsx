  ...
  return (
    <Section heading="Current user">
      <div className="panel" onMouseDown={(e) => e.stopPropagation()}>
        <select onChange={(e) => select(e.target.value)}>{list}</select>
        <button className="add-item" onClick={(_e) => add()}>
          &#x002B;
        </button>
      </div>
    </Section>
    ...
    <Section heading="Watched queries">
      <div className="panel" onMouseDown={(e) => e.stopPropagation()}>
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") show(query);
          }}
        />
        <button className="add-item" onClick={(_e) => show(query)}>
          &#x002B;
        </button>
      </div>
      {errorComponent}
  ...
  return (
    <div className="skdb-table" onMouseDown={(e) => e.stopPropagation()}>
      <div className="skdb-table-header">
        ...
        <button className="close-query" onClick={(_e) => removeQuery(query)}>
          &#x2718;
        </button>
      </div>
...
function Draggable({ children }) {
  ...
  useEffect(() => {
    if (ref.current) {
      if (position.x !== undefined || position.y !== undefined) {
        const right =
          window.innerWidth - ((position.x || 0) + ref.current.offsetWidth);
  ...
  return (
    <div
      ref={ref}
      className="skdb-dev-console-draggable"
  ...
  useEffect(() => {
    skdbAsRoot.connectedRemote().then((remote) => {
      ...
      remote.exec("SELECT userID FROM skdb_users").then((userRows) => {
        setUsers(userRows.onlyColumn() as string[]);
      });
    });
  }, []);
