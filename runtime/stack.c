...
void sk_stack3_init(stack3_t* st, size_t capacity) {
  st->head = 0;
  st->capacity = capacity;
  st->values = (value3_t*)sk_malloc(sizeof(value3_t) * capacity);
}

void sk_stack3_free(stack3_t* st) {
  sk_free_size(st->values, sizeof(value3_t) * st->capacity);
}

void sk_stack3_grow(stack3_t* st) {
  stack3_t new_st;
  new_st.head = 0;
  new_st.capacity = st->capacity * 2;
  new_st.values = (value3_t*)sk_malloc(sizeof(value3_t) * new_st.capacity);
  for(; new_st.head < st->capacity; new_st.head++) {
    new_st.values[new_st.head] = st->values[new_st.head];
  }
  sk_free_size(st->values, sizeof(value3_t) * st->capacity);
