...
- payload
  ...
  - client_version_length (8)
  - client_version (client_version_length * 8) - ASCII encoded string identifying this client
...
Result sets do not have an end of data marker as the process
...
The JS client will throw an SKDBRebootException. Applications are
expected to catch this and reload entirely or at least re-create their
SKDB client. Reloading entirely is likely preferable as this may
signal a schema change which the app should update to accommodate.
