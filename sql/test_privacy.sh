...
if echo -e "1\t234" | $SKDB write-csv t2 --user user1 2>&1 | grep -q Error; then
...
if echo -e "0\t234" | $SKDB write-csv t2 --user user1 2>&1 | grep -q Error; then
...
tail -f /dev/null |
  $SKDB tail $subt1 --format=csv --user user3 --follow |
