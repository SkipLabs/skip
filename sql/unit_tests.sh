...
if cat test/test_unique.sql | ../build/skdb 2>&1 | grep -q UNIQUE
then
...
if cat test/test_concat.sql | ../build/skdb | grep -q hellohello
then
...
if cat test/test_notnull.sql | ../build/skdb 2>&1 | grep -q NULL
then
...
if cat test/test_delete.sql | ../build/skdb | grep -v -q 22
then
...
cat test/test_like.sql | ../build/skdb > /tmp/kk1
...
if cat test/join_select_order.sql | ../build/skdb --always-allow-joins | grep -q '4|6|4|4|1|4'
then
...
if cat test/test_update.sql | ../build/skdb | grep -q '2|3'
then
    echo -e "TEST JOIN ORDER:\tOK"
else
    echo -e "TEST JOIN ORDER:\tFAILED"
