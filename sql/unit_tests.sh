...
if cat test/unit/test_concat.sql | $SKDB | grep -q hellohello
then
...
if cat test/unit/test_delete.sql | $SKDB | grep -v -q 22
then
...
cat test/unit/test_like.sql | $SKDB > /tmp/kk1
...
if cat test/unit/join_select_order.sql | $SKDB --always-allow-joins | grep -q '4|6|4|4|1|4'
then
...
if cat test/unit/test_update.sql | $SKDB | grep -q '2|3'
then
    echo -e "TEST JOIN ORDER:\tOK"
else
    echo -e "TEST JOIN ORDER:\tFAILED"
...
if cat test/unit/primary_index.sql | $SKDB --show-used-indexes | grep -q 'USING INDEX: t1_id'
then
...
if cat test/unit/test_not_like.sql | $SKDB | grep -q 'def'
then
...
if cat test/unit/aggr_union.sql | $SKDB | awk '{x+=$1}; END {print x}' | grep -q '11'
then
...
cat test/unit/join_outside_of_virtual.sql | $SKDB 2> /tmp/kk1
...
if cat test/unit/insert_autoid.sql | $SKDB | grep -q "3"
then
...
if cat test/unit/lower_upper.sql | $SKDB | grep -q "FOO|bar"; then
...
if cat test/unit/test_string_escaping_json.sql | $SKDB --format=json | grep -q '{"a":"\\"Hello world\\""}'; then
    echo -e "TEST STRING ESCAPING JSON:\tOK"
else
    echo -e "TEST STRING ESCAPING JSON:\tFAILED"
fi

if cat test/unit/test_null_encoding_json.sql | $SKDB --format=json | grep -q '{"a":1,"b":null,"c":2}'; then
    echo -e "TEST NULL ENCODING JSON:\tOK"
else
    echo -e "TEST NULL ENCODING JSON:\tFAILED"
fi

if cat test/unit/test_real.sql | $SKDB | grep -q "1"; then
    echo -e "TEST REAL:\tOK"
else
    echo -e "TEST REAL:\tFAILED"
fi

if cat test/unit/test_missing_semi.sql | $SKDB | grep -q "22"; then
    echo -e "TEST MISSING SEMI:\tOK"
else
    echo -e "TEST MISSING SEMI:\tFAILED"
fi

if cat test/unit/test_ignore_replace.sql | $SKDB | tr '\n' 'S' | grep -q "22|23S24|26S25|27S31|32S33|34S51|42|43S"; then
    echo -e "TEST IGNORE/REPLACE:\tOK"
else
    echo -e "TEST IGNORE/REPLACE:\tFAILED"
fi

if cat test/unit/test_ignore_replace2.sql | $SKDB | tr '\n' 'S' | grep -q "50|1S50|2S"; then
    echo -e "TEST IGNORE/REPLACE2:\tOK"
else
    echo -e "TEST IGNORE/REPLACE2:\tFAILED"
fi

if cat test/unit/test_underscore_alias.sql | $SKDB | grep -q "22"; then
    echo -e "TEST UNDERSCORE ALIAS:\tOK"
else
    echo -e "TEST UNDERSCORE ALIAS:\tFAILED"
fi

if cat test/unit/create_index_if_not_exists.sql | $SKDB | grep -q "22"; then
...
if cat test/unit/test_select_star_plus.sql | $SKDB | grep -q "22|23|45"; then
...
if cat test/unit/test_count_string.sql | $SKDB | grep -q "0"; then
...
if cat test/unit/limit_unit.sql | $SKDB | wc | tr ' ' 'S' | tr '\n' 'S' | grep -q "SSSSSS1SSSSSSS1SSSSSSS2S"; then
    echo -e "TEST LIMIT:\tOK"
else
    echo -e "TEST LIMIT:\tFAILED"
fi

if cat test/unit/test_index_transaction.sql | $SKDB --show-used-indexes | tr ' ' 'S' | grep -q "USINGSINDEX:ST1_a"; then
    echo -e "TEST TRANSACTION INDEX:\tOK"
else
    echo -e "TEST TRANSACTION INDEX:\tFAILED"
fi

if cat test/unit/test_index_transaction2.sql | $SKDB --show-used-indexes | tr ' ' 'S' | grep -q "USINGSINDEX:ST1_a"; then
