...
const manageMd = (md: string) => {
  ...
  const dist = relative("src", md, "./dist");
...
const manageScss = (scss: string, css: string) => {
  ...
  if (deps) {
    deps.forEach(f => {
      if (f.endsWith(".scss")) {
        const css = relative("src", mvext(f, ".css"), "./dist")
        manageScss(f, css);
      }
    });
  }
...
const onChange2 = (f: string) => {
  ...
  if (ext == ".md") {
    ...
    let target = relative("src", f, "./dist");
    fs.cp(f, target, (err) => {
      if (err) {
        console.error(err);
      }
...
const onUnlink2 = (f: string) => {
  ...
  if (ext == ".md") {
    ...
    const dist = relative("src", f, "./dist");
    const html = mvext(dist, ".html");
...
const post = (response: http.ServerResponse, file: string, errorFile: string, contentType: string) => {
...
const requestHandler = (request: http.IncomingMessage, response: http.ServerResponse) => {
  ...
  if (request.url == "/__version__/") {
    ...
    if (request.url == "/") {
      ...
      page = ("./dist" + request.url).replace(/\/+$/, '') + ".html";
    }
    post(response, page, error, "text/html");
  }
}
