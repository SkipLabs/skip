...
class LinksImpl implements Links {
  ...
  complete = (utils: Utils, exports: object) => {
    ...
    this.SKIP_localetimezonename = (year: int, month: int, day: int) => {
      let date = new Date(year, month - 1, day);
      const timeZone = date.toLocaleDateString(undefined, { timeZoneName: "short" });
    ...
    this.SKIP_locale = (code: int, value: int) => {
      ...
      if (str == "B" || str == "b") {
        ...
        let name =  date.toLocaleDateString(undefined, { month: str == "b" ? 'short' : 'long'});
        return utils.exportString(name);
      }
      if (str == "A" || str == "a") {
        let date = new Date(Date.UTC(2023, 8, 18 + (value - 1))); // A monday
        let name =  date.toLocaleDateString(undefined, { weekday: str == "a" ? 'short' : 'long'});
        return utils.exportString(name);
      }
      if (str == "p") {
        let hour = 10;
        ...
        let m = date.toLocaleDateString(undefined, { hour12: true, hour: 'numeric' }).split(" ");
...
class Manager implements ToWasmManager {
  ...
  prepare = (wasm: object) => {
    ...
    toWasm.SKIP_localetimezone = (year: int, month: int, day: int) => links.SKIP_localetimezone(year, month, day); 
    toWasm.SKIP_localetimezonename = (year: int, month: int, day: int) => links.SKIP_localetimezonename(year, month, day);
    toWasm.SKIP_locale = (code: int, value: int) => links.SKIP_locale(code, value);
